...
Exact commands to produce a 3-minute demo video. Pair with `narration.md` for voiceover/caption text.

## What you get

- Automated, repeatable recording of four beats (`apps/web/demo/output/*.webm`) — no manual clicking.
- Two static artifact pages captured by the recorder:
  `apps/web/demo/metrics.html` (chaos P95 + ablation table) and
  `apps/web/demo/trace.html` (guardrail/agent trace, including cross-tenant PermissionError).
...
# auto-generates fake-backend ablation so there is always content:
...
Chat beats (1–2) need the app running. For a deterministic, fast take, run the API on the fake backend:
...
For an authentic take with real model latency, start the API normally (`make api` with Ollama running).

Videos land in `apps/web/demo/output/record-*/video.webm`.

> If the web app is not running, the recorder still captures beats 3–4 from the artifact pages and produces video (chat beats graceful skip).

## Step 4 — Post & publish

```bash
# convert + (optional) add title card
ffmpeg -i apps/web/demo/output/record-*/video.webm docs/demo/resolveai-demo.mp4
```

Upload to Loom / YouTube, add voiceover from `narration.md`, paste the link into the resume bullet (see `docs/milestone-8-plan.md`).
