...
# --------------------------------------------------------------------------- #
# Reproducibility snapshots (speed-independent: captures *what* ran, not how
# fast). These let a reader reconstruct the run without re-executing it. The
# heavy lifting lives in resolveai_api.eval.run_meta so eval scripts and this
# orchestrator stay in sync.
# --------------------------------------------------------------------------- #

from resolveai_api.eval.run_meta import (  # noqa: E402
    env_snapshot as _env_snapshot,
)
from resolveai_api.eval.run_meta import (  # noqa: E402
    fixture_snapshot as _fixture_snapshot_for,
)
from resolveai_api.eval.run_meta import (  # noqa: E402
    model_snapshot as _model_snapshot,
)


def _fixture_snapshot() -> dict[str, Any]:
    return _fixture_snapshot_for(
        {
            "red_team": ROOT / "apps/api/tests/fixtures/red_team.jsonl",
            "benign": ROOT / "apps/api/tests/fixtures/benign_tickets.jsonl",
            "benchmark": ROOT / "apps/api/tests/fixtures/benchmark_tickets.jsonl",
            "kb_retrieval_golden": ROOT / "apps/api/tests/fixtures/kb_retrieval_golden.jsonl",
        }
    )


class RunState:
    """Phase state machine with a machine-readable status mirror.

    Every phase moves pending -> running -> ended|failed|aborted, and is flushed
    to disk on each transition. The guarantee: a `running` phase that never
    reaches a terminal state means the orchestrator died mid-phase — which is
    exactly what `run_status.json` / manifest `is_complete=false` now make
    explicit instead of leaving a reader to infer it from a dangling `START` log.
    """

    def __init__(self, out_dir: Path, *, argv: list[str]) -> None:
    ...
    def start_phase(self, milestone: str, name: str) -> dict[str, Any]:
        phase = {
            "milestone": milestone,
            "name": name,
            "status": "running",
        ...
        self.manifest["phases"].append(phase)
        self._current = phase
        print(f"[e2e] START {milestone}/{name} {phase['started_at']}", flush=True)
        self.flush()
        return phase

    def finish_phase(
        self, phase: dict[str, Any], *, exit_code: int, elapsed_s: float, **extra: Any
    ) -> None:
        phase["status"] = "ended" if exit_code == 0 else "failed"
        phase["exit_code"] = exit_code
        phase["elapsed_s"] = round(elapsed_s, 2)
        phase["finished_at"] = _ts()
        phase.update(extra)
        if phase is self._current:
            self._current = None
        print(
            f"[e2e] {phase['status'].upper()} {phase['milestone']}/{phase['name']} "
            f"exit={exit_code} elapsed={elapsed_s:.1f}s",
            flush=True,
        )
        self.flush()
    ...
    def abort(self, reason: str) -> None:
        if self._current is not None and self._current.get("status") == "running":
            ...
            self._current["finished_at"] = _ts()
            print(
                f"[e2e] ABORTED {self._current['milestone']}/{self._current['name']} "
                f"reason={reason}",
                flush=True,
            )
        self.manifest["aborted_at"] = _ts()
        self.manifest["incomplete_reason"] = reason
        self.manifest["is_complete"] = False
        self.flush()

    def complete(self) -> None:
        self.manifest["finished_at"] = _ts()
        unfinished = [
            p for p in self.manifest["phases"] if p.get("status") not in ("ended", "failed")
        ]
        self.manifest["is_complete"] = (
            not unfinished and self.manifest.get("incomplete_reason") is None
        )
        self.flush()

    def has_blocking_failures(self) -> bool:
        # api_health is best-effort (API may be offline during eval-only runs).
        return any(
            p.get("status") in ("failed", "aborted") and p.get("name") != "api_health"
            for p in self.manifest["phases"]
        )

    def flush(self) -> None:
        _write_json(self.out_dir / "manifest.json", self.manifest)
        _write_json(self.out_dir / "run_status.json", self._status_view())
    ...
    def _status_view(self) -> dict[str, Any]:
        return {
            ...
            "is_complete": self.manifest.get("is_complete", False),
            "incomplete_reason": self.manifest.get("incomplete_reason"),
            "phases": [
                {
...
def main() -> int:
    ...
    state = RunState(out_dir, argv=sys.argv[1:])
    print(f"[e2e] output → {out_dir}")

    try:
        # M1 — health
        phase = state.start_phase("M1", "api_health")
        code, elapsed = _run_cmd(
            ["curl", "-sf", "http://127.0.0.1:8000/"],
            log_path=out_dir / "raw" / "m1_health.log",
        )
        state.finish_phase(phase, exit_code=code, elapsed_s=elapsed)

        # M2 — live LLM unit/integration tests
        phase = state.start_phase("M2", "llm_live_pytest")
        code, elapsed = _run_cmd(
            [
                "uv",
                "run",
        ...
        phase = state.start_phase("M2", "live_supervisor")
        ...
        phase = state.start_phase("M3", "toolbelt_discovery")
        code, elapsed = _run_cmd(
            [
                "uv",
                "run",
        ...
        if not args.skip_m5:
            phase = state.start_phase("M5", "eval_adversarial")
        ...
        state.abort("KeyboardInterrupt")
        _write_run_readme(state)
        print(f"[e2e] ABORTED → {out_dir / 'README.md'}")
        return 130
    except Exception as exc:
        state.abort(f"{type(exc).__name__}: {exc}")
        _write_run_readme(state)
        print(f"[e2e] FAILED → {out_dir / 'README.md'}")
        raise

    state.complete()
    _write_run_readme(state)
    print(f"[e2e] done → {out_dir / 'README.md'}")
    return 1 if state.has_blocking_failures() else 0


def _write_run_readme(state: RunState) -> None:
    ...
    report_lines = [
        ...
        f"- **Finished**: {manifest.get('finished_at') or '(unfinished)'}",
        f"- **Complete**: {manifest.get('is_complete')}",
    ]
    if manifest.get("incomplete_reason"):
        report_lines.append(f"- **Incomplete reason**: `{manifest['incomplete_reason']}`")
    report_lines.extend(
        [
            "",
            "## Phases",
            "",
            "| Milestone | Phase | Status | Exit | Seconds |",
            "|---|---|---|---:|---:|",
        ]
    )
    for ph in manifest["phases"]:
        report_lines.append(
            f"| {ph.get('milestone')} | {ph.get('name')} | {ph.get('status')} | "
            f"{ph.get('exit_code') if ph.get('exit_code') is not None else '—'} | "
            f"{(ph.get('elapsed_s') or 0):.1f} |"
        )
    report_lines.extend(
        [
            "",
            "## Artifacts",
            "",
            "- `manifest.json` — full run manifest (argv, env, models, fixtures, phases)",
            "- `run_status.json` — machine-readable phase state + `is_complete`",
